...
const getEvents = async () => {
  ...
  //         <img src="profile/add.jpg" style="width: 7%;" align="right" onclick="location.href='write-message.html?id=${
  //           event.id
  //         }'" />
  //         <hr /></label>`;
  //   getById('today').innerHTML += eventText;
  ...
  if (json.todays.eventsCount !== 0) {
    json.todays.events.forEach((event) => {
      const eventText = `<label class="no-header-label"><span onclick="location.href='edit-birthday.html?id=${
        event.id
      }'">${event.target}</span>
          <img src="profile/add.jpg" style="width: 7%;" align="right" onclick="location.href='write-message.html?id=${
            event.id
